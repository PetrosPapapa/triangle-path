...
object Triangle {
  ...
  /**
    ...
    *
    * Starting from the [[Path]]s of the previous (lower) row, we calculate the
    * minimal step from the current row.
    *
    * @param results
    *   the result of the previous row
    * @param row
    *   the current row
    * @return
    *   the minimal paths for the current row
    */
  ...
  /**
    ...
    *
    * May return multiple results in the sequence if given an invalid triangle.
    *
    * @note
    *   Expects the rows in reverse order (bottom row first).
    * @param rows
    *   The rows of the triangle (bottom row first).
    * @return
    *   A sequence containing the minimal path.
    */
  def minPath(triangle: Triangle): Option[Path] = triangle.headOption.flatMap {
    r =>
      foldAll(triangle.tail, r.values.map(Path(_)))
  }

  @tailrec
  private def foldAll(rows: Seq[Row], acc: Seq[Path] = Seq()): Option[Path] =
